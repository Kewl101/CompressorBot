...
async def screenshot(e):
    ...
    tsec = await genss(dl)
    fps = 10 / tsec
    ncmd = f"ffmpeg -i '{dl}' -preset ultrafast -vf fps={fps} -vframes 10 '{wah}/pic%01d.png'"
...
async def _720px265(e):
    try:
        ...
        cmd = f'ffmpeg -y -i "{dl}" -c:v libx265 -vf scale=1280:-2 -preset superfast -crf 28 -x265-params aq-moode=3 -c:a libfdk_aac -profile:a aac_he -ac 2 -vbr 3 -c:s copy -map 0 "{out}"'
        process = await asyncio.create_subprocess_shell(
...
async def _480px265(e):
    try:
        ...
        cmd = f'ffmpeg -y -i "{dl}" -c:v libx265 -vf scale=854:-2 -preset fast -crf 27 -x265-params aq-moode=3 -c:a libfdk_aac -profile:a aac_he -ac 2 -vbr 3 -c:s copy -map 0 "{out}"'
        process = await asyncio.create_subprocess_shell(
            cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
        )
        stdout, stderr = await process.communicate()
...
async def _480px264(e):
    try:
        ...
        cmd = f'ffmpeg -y -i "{dl}" -c:v libx264 -vf scale=854:-2 -preset medium -crf 20 -x265-params aq-moode=3 -c:a libfdk_aac -profile:a aac_he -ac 2 -vbr 3 -c:s copy -map 0 "{out}"'
        process = await asyncio.create_subprocess_shell(
            cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
        )
        stdout, stderr = await process.communicate()
...
async def _720px265s(e):
    ...
    xxx = await e.edit(
        "`Generating Sample of How Compressed Video Looks...`",
        buttons=[
            [Button.inline("STATS", data=f"stats{wah}")],
            [Button.inline("CANCEL PROCESS", data=f"skip{wah}")],
        ],
    )
    ncmd = f'ffmpeg -y -i "{dl}" -ss {ss} -to {dd} -c:v libx265 -vf scale=1280:-2 -preset superfast -crf 28 -x265-params aq-moode=3 -c:a libfdk_aac -profile:a aac_he -ac 2 -vbr 3 -c:s copy -map 0 "{out}"'
    process = await asyncio.create_subprocess_shell(
        ncmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
    )
    stdout, stderr = await process.communicate()
...
async def _480px265s(e):
    ...
    ncmd = f'ffmpeg -y -i "{dl}" -ss {ss} -to {dd} -c:v libx265 -vf scale=854:-2 -preset fast -crf 27 -x265-params aq-moode=3 -c:a libfdk_aac -profile:a aac_he -ac 2 -vbr 3 -c:s copy -map 0 "{out}"'
    process = await asyncio.create_subprocess_shell(
        ncmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
    )
    stdout, stderr = await process.communicate()
...
async def _480px264s(e):
    ...
    ncmd = f'ffmpeg -y -i "{dl}" -ss {ss} -to {dd} -c:v libx264 -vf scale=854:-2 -preset medium -crf 20 -x265-params aq-moode=3 -c:a libfdk_aac -profile:a aac_he -ac 2 -vbr 3 -c:s copy -map 0 "{out}"'
    process = await asyncio.create_subprocess_shell(
        ncmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
    )
    stdout, stderr = await process.communicate()
...
async def encod(event):
    try:
        ...
        pp = []
        async for x in event.client.iter_participants("ensemblygroup2"):
            pp.append(x.id)
        if (user.id) not in pp:
            return await xxx.edit(
                "U Must Subscribe This Channel To Use This Bot",
                buttons=[Button.url("JOIN CHANNEL", url="t.me/ensemblygroup2")],
            )
        if len(COUNT) > 3 and user.id != OWNER:
            llink = (await event.client(cl(LOG))).link
            return await xxx.edit(
                "`Overload Already 3 Process Running`",
        ...
        if not os.path.isdir(rr):
            os.mkdir(rr)
        bb = kk.replace(f".{aa}", " compressed @ensembly.mkv")
        out = f"{rr}/{bb}"
        thum = "thumb.jpg"
        ...
        await event.client.send_message(
            ...
            buttons=[
                ...
                [Button.inline("COMPRESS", data=f"encc{key}")],
            ],
        )
